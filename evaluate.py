...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional path to save the results table as CSV (e.g. output/scores.csv). "
             "Console output is still printed either way."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Only score these relations (comma-separated). Without this, rows "
             "of other relations that are absent from the predictions file are "
             "counted as empty abstentions and pollute the overall averages."
    )
    ...
    if args.relation:
        ...
        available = sorted({r["Relation"] for r in gt_rows})
        unknown = sorted(relations - set(available))
        if unknown:
            raise SystemExit(
                f"Relation(s) {unknown} not present in {args.ground_truth}. "
                f"Available: {available}"
            )
        gt_rows = [r for r in gt_rows if r["Relation"] in relations]
        pred_rows = [r for r in pred_rows if r["Relation"] in relations]
        print(f"Scoring {len(gt_rows)} row(s) for relation(s): {sorted(relations)}\n")
    ...
    if args.output:
        ...
        results.to_csv(output_path, index_label="Relation")
